        ...
        private void LVitems_ContextMenu_ContextMenuOpening(object sender, ContextMenuEventArgs e)
        {
            ...
            foreach (ContextMenuDataModel data in LV_items.ContextMenu.Items)
            {
                ...
                switch (data.Key)
                {
                    ...
                    case LanguageKey.TSMI_copyid: if (selected_count != 1 || managerehistory_itemnodes.Root.RootType.Type == CloudType.LocalDisk) data.IsEnabled = false; else data.IsEnabled = true; break;
