...
namespace Cloud.Dropbox
{
  public class DropboxRequestAPIv2
  {
    ...
    private T POST_Request<T>(string url, object obj_request = null, string[] headers = null, string ContentType = "application/json", bool upload = false)
    {
      ...
      {
        if (!upload) st = rq.GetDataResponse();//download
        else st = rq.UploadData();//upload
        return (T)(object)st;
      }
      else//text
      {
        string text_response = rq.GetTextResponse();//.Replace("\".tag\":", "\"tag\":");
    ...
    {
      List<string> headers = new List<string>() { "Dropbox-API-Arg: " + JsonConvert.SerializeObject(session) };
