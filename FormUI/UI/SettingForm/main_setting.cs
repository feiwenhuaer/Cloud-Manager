...
namespace FormUI.UI.SettingForm
{
  public partial class main_setting : UserControl
  {
    ...
    public void Save()
    {
      ...
      Setting_UI.reflection_eventtocore.SettingAndLanguage.SetSetting(SettingsKey.AutoStartTransfer, CB_AutoStartTransfer.Checked ? "1" : "0");
      Setting_UI.reflection_eventtocore.SettingAndLanguage.SetSetting(SettingsKey.AutoLogin, CB_autologin.Checked ? "1" : "0");
      Setting_UI.reflection_eventtocore.SettingAndLanguage.SetSetting(SettingsKey.ShutdownWhenDone, CB_shutdown.Checked ? "1" : "0");

      if (!string.IsNullOrEmpty(CBB_lang.Text) && CBB_lang.Text != langfilename)
      {
        Setting_UI.reflection_eventtocore.SettingAndLanguage.SetSetting(SettingsKey.lang, CBB_lang.Text);
        ChangeUI = true;
      }
      if (!string.IsNullOrEmpty(CBB_ui.Text) && CBB_ui.Text != uifilename)
      {
        Setting_UI.reflection_eventtocore.SettingAndLanguage.SetSetting(SettingsKey.UI_dll_file, CBB_ui.Text);
        ChangeLang = true;
      }

      Setting_UI.reflection_eventtocore.SettingAndLanguage.SetSetting(SettingsKey.MaxGroupsDownload, NUD_group.Value.ToString());
      Setting_UI.reflection_eventtocore.SettingAndLanguage.SetSetting(SettingsKey.MaxItemsInGroupDownload, NUD_item.Value.ToString());
      Setting_UI.reflection_eventtocore.SettingAndLanguage.SetSetting(SettingsKey.BufferSize, NUD_buffer.Value.ToString());
      Setting_UI.reflection_eventtocore.SettingAndLanguage.SetSetting(SettingsKey.GD_ChunksSize, NUD_GDchunks.Value.ToString());
      Setting_UI.reflection_eventtocore.SettingAndLanguage.SetSetting(SettingsKey.Dropbox_ChunksSize, NUD_DBchunks.Value.ToString());
    ...
    public bool ChangeUser_Pass()
    {
      ...
      if (CanSave)
      {
        return Setting_UI.reflection_eventtocore.AccountsAndCloud.ChangeUserPass(TB_username.Text, TB_oldpass.Text, TB_newpass0.Text);
      }
      return true;
    }
  }
